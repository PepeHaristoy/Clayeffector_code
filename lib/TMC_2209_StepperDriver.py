from lib.TMC_2209_uart import TMC_UART
import lib.TMC_2209_reg as reg
...
class TMC_2209:
    ...
    _maxSpeedHoming = 600          # the maximum speed in steps per second for homing
...
# constructor
#-----------------------------------------------------------------------
    def __init__(self, pin_step, pin_dir, pin_en, rx_pin, tx_pin, mtr_id=0, baudrate=115200, serialport=0):
...
#-----------------------------------------------------------------------
    def doHoming(self, direction, threshold=None):
        ...
        if (direction == 1):
            self._targetPos = self._stepsPerRevolution * 30
            print(self._targetPos)  # debug
            targetPos = self._targetPos
        else:
            self._targetPos = -self._stepsPerRevolution * 30
            print(self._targetPos)  # debug
            targetPos = self._targetPos
        ...
        #self.setMaxSpeed(8000) #(self._maxSpeedHoming)
        print(self._maxSpeed)
        self.computeNewSpeed()
        step_counter=0
        print("TMC2209: Steps per Revolution: "+str(targetPos))
        while (step_counter<abs(targetPos)):
...
#-----------------------------------------------------------------------
    def setCurrent(self, run_current, hold_current_multiplier = 0.5, hold_current_delay = 10, Vref = 1.3):
        CS_IRun = 0
        Rsense = 0.11
        Vfs = 0
